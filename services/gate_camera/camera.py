...
def _gate_static_path_is_vehicle_roi(url: str) -> bool:
    """True if this URL points to the temporary full-vehicle crop saved at line-cross IN/OUT."""
    if not url:
        return False
    return "_vehicle_in_" in url or "_vehicle_out_" in url
...
def process_gate_video_stream(video_url, socketio=None, gate_ocr_results_dict=None, process_interval=5):
    ...
    # Một gate_log chỉ cần backfill ảnh crop biển một lần (tránh spam DB khi OCR lặp frame)
    _plate_crop_backfilled_gate_logs: set = set()
    _PLATE_CROP_BACKFILL_CAP = 4000
    ...
    def _upsert_plate_fifo_for_track(track_id, plate_text: str, plate_conf: float, image_path: str = None, ocr_ts: int = None) -> bool:
        ...
        with plate_fifo_lock:
            ...
            # Resolve gate_log_id for any DB backfill (plate and/or plate-crop image)
            ...
            # OCR improved plate/conf → cập nhật DB (kèm ảnh nếu có)
            if plate_improved and gate_log_id:
                runtime.submit_io(
                ...
                gate_log_id
                and image_path
                and not plate_improved
                and not _gate_static_path_is_vehicle_roi(image_path)
                and prev_plate
                and str(prev_plate).strip().upper() == str(plate_text).strip().upper()
                and gate_log_id not in _plate_crop_backfilled_gate_logs
            ):
                # Biển đã khớp FIFO/DB từ lúc qua vạch nhưng ảnh DB vẫn là ROI xe;
                # OCR sau đó mới có file crop biển → vẫn phải ghi image_path.
                _plate_crop_backfilled_gate_logs.add(gate_log_id)
                if len(_plate_crop_backfilled_gate_logs) > _PLATE_CROP_BACKFILL_CAP:
                    _plate_crop_backfilled_gate_logs.clear()
                runtime.submit_io(
                    update_gate_entry_plate,
                    gate_log_id,
                    session_id,
                    ...
                    image_path=image_path,
                    coalesce_group="gate_entry_update",
                    coalesce_key=f"{gate_log_id}:lateimg:{ocr_ts}" if ocr_ts else f"{gate_log_id}:lateimg",
                )
                print(
                    f"[GATE OCR] Late plate-crop image backfill log_id={gate_log_id} "
                    f"track={track_id} img={image_path}"
                )
